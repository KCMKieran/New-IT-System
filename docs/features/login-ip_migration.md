...
## 8.5 后续探索：共享 IP 跨客户聚合（2026-04-24，not-in-prod）

同事提出新需求：

> 在系统的「Login IP 监测」中，新增并显示**所有客户**的交易 IP，以便风控监控。
> 检测规则：不同 UID + 使用相同 IP + 交易开仓前后差 10 秒 + 相同产品 + 反方向交易。

### 评估结论

原始需求要做"**10 秒内反方向同产品**"需要从 MT journal 日志里提取每笔开仓事件的 IP/时间/方向/产品。抽样 `backend/data/login_ip/tmp/*/*.log` 确认：

- MT4 日志里有 `'ACCOUNT': market buy 0.30 EURUSD ...` 这种带 IP + 账户 + 方向 + 产品的行
- MT5 同理（utf-16-le，多两列偏移），与现有 `login_ip_analyzer_service` 的 parse 框架兼容

**但是**：单台 MT 日志 2–3GB/天，解析开仓事件要多抓多存，数据量翻倍；且 10s 窗口 × 反方向 × 相同产品的精确匹配对 "UID 映射是否完整" 高度敏感。跟项目 owner 对齐后**第一步先走轻量路线**：

### 轻量方案（现行）

只用现有 `analysis_ip_to_accounts.json`（login 事件聚合产物，不碰原始日志），回答一个缩小版问题：
...
| 服务器 | 总 IP | 共享 IP (≥2 真实客户) | 涉及客户 | 最多客户/IP |
|--------|------|----------------------|---------|------------|
| MT4 | 4,095 | 430 | 547 | 41（`124.220.165.142` 腾讯云，38 UID） |
| MT4_Live2 | 513 | 69 | 91 | 7 |
| MT5 | 4,495 | 539 | 559 | 36（`116.96.46.10` 越南，36 UID 完全无重叠）|
| **合计** | 9,103 | **1,038** | 1,197 | — |

`--known-accounts-only` 的 CRM 白名单阶段加载了 **94,970 个 MT LOGIN**（`fxbackoffice.mt4_users.userId IS NOT NULL`），用于在脚本里过滤掉 demo/测试/无绑定客户的 MT 账号。

### 头部 IP 读数

- `124.220.165.142` 14 天有 13 天出现，累计 78 不同客户 —— 大概率 **腾讯云 IDC 出口 / 代理池**，不是真实客户家宽
- `22022xxx` / `55xxx` 段的 MT4_Live2 账户在多个美国 IDC IP (`45.39.9.216` / `23.26.83.192` / `168.199.222.245` ...) 上轮转出现 —— 疑似**同一批托管账户 + 轮换代理**
- `116.96.46.10` / `116.96.46.255` 两个越南 IP 分别关联 MT5 cent 账号 `60xxxxx` 和标准账号 `67xxxxx`，混用

### 下一步决策点（未落地）

1. **IP 归属自动标注**：ipinfo.io 免费层 (50k/mo) 够用，给 CSV 加 `is_hosting / asn_name / country`，机房 IP 天然是"出口"不算异常
2. **内部忽略白名单**：风控看过后把确认是公司出口 / 员工休息室 IP 加到 SQLite 某张新表，脚本自动排除
3. **严格对冲检测**（原始需求 10s 反方向）：等轻量方案跑顺 1–2 周、确认风控确实用得上再评估扩到交易日志
4. **前端落地**：若决定要做，新开 "共享 IP" Tab（和现有 4 个 Tab 并列），API `GET /api/v1/login-ip/shared-ips?date=`，复用异步 CSV 导出模式

此节标记为 **not-in-prod**：脚本只在 `backend/scripts/` 下做 ad-hoc 使用，没接调度、没入前端、没进 API。任何往生产推的动作需要新开一轮 Phase。
