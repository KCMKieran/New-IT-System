...
- **review F5（live-with）**：`get_account_info_map` 查不到 currency 时默认 USD，CEN 账户在 `floor>0` 规则下浮亏可能放大 100× 误报。考虑 currency miss 且 floor>0 时 fail-closed（跳过该账户）。当前仅误报、仅 floor>0、且符合既定 default-USD 约定，未改。
- **前端 tab 放出**：当前 `MARTINGALE_TAB_VISIBLE=false`（后端已上线，前端隐藏）。验证后改 true + 取消 `RISK_MONITOR_TABS` / `RISK_MONITOR_TAB_LABELS` 两处注释即放出。

## 结果（2026-06-03 done）

**实际交付 vs AC**：全部 AC 达成。后端 `rule_martingale_service.py`（event-gated，复用 opens GATE + 持仓快照按 `(login,symbol,direction)` 分组）+ `alert_martingale_detail` 表 + 4 endpoint + scheduler slow/all tier；schema `MartingaleConfig`/`MartingaleRule` + AlertEvent 6 字段（`lot_ratio_mg` 避开 gap 的 `lot_ratio`）；前端 `MartingaleTab`（标准 4 hook + per-rule 卡片 + config drawer + 立即扫描 + 9 专属列）。19 单测 + DB round-trip + scheduler tier 测试更新。`verify.sh` 三硬闸全绿。三 endpoint 对 live DB smoke-test 通过。

**与计划的偏差**：
- **前端 tab 隐藏上线**（用户 2026-06-03 要求）：后端 live + 前端 `MARTINGALE_TAB_VISIBLE=false` 守卫，待验证后一行放出。
- 倍数判定语义：review F3 后从「最新加仓」改为「最大加仓」（见下）。

**Stage 1 outsider-review（8 finding）处理记录**：
- **F3（当场修）**：一个扫描窗内多笔加仓只评估最新一笔 → 大额中间加仓（1.0→10.0→1.0）漏判且永久不重评估。改成倍数判定用 ladder 最大非建仓加仓手数 + 2 单测。commit `cb71f9f`。
- **F2（live-with + 注释）**：建仓笔 = 最早**存活**腿，部分平仓后 re-anchor。是「持仓口径」的有意后果，加代码注释 + pin 测试。
- **F1（live-with）**：持仓快照 `LOGIN IN (...)` 无上限 — 与已在 prod 的 Quick Profit 浮动查询完全同款，扩容一起优化。
- **F5（live-with → follow-up）**：currency miss 默认 USD → CEN floor>0 可能 100× 误报（见 Follow-up）。
- **F4/F6/F7（live-with）**：MT5 `/10000` 是全项目既定除数（无规则做 SQL 层测试）；`direction` 命名 + mega-SELECT 是 band 121+ 该处理的既有架构 smell；重启 seed 窗口与 leverage abuse 一致。
