...
### 1. 账户范围不一致 —— 最重要，且是**现存**问题

**现行 `profit_hist` 子查询没有任何 sid / demo 过滤**
（`client_return_service.py:338-345`，`FROM stats_trading_running_totals WHERE userId IN (...)`），
而 `avg_daily_equity` 有 `sid IN (1,5,6) AND GROUP NOT LIKE '%demo%'`。
...
| 两种口径结果不同的客户 | **2,044** |
| 差 > $1,000 | **540** |
| 全部账户都在 ROACE 范围之外（有 profit_hist，无 avg_daily_equity） | **1,705** |

现行 ROACE 忍得了（只是分子分母略微不同源）；**新公式忍不了** ——
`Closed + ΔFloating ≡ ΔEquity − 净流入` 这个恒等式只在两边取同一批账户时成立。

**必须统一。但统一之后现有 ROACE 列的数值会变**（540 个客户变动超 $1,000）。

> 待决策：**(a)** 统一到 `sid IN (1,5,6)` 非 demo（推荐，与分母一致），接受 ROACE 数值变化；
> **(b)** 只给新列用严格口径、ROACE 保持现状（两列口径不同，需在 tooltip 写明）；
> **(c)** 先只修口径不加列（纯 bug fix，降为 S）。

### 2. 首日浮动（`Floating_start`）怎么取

实测 25,553 个客户中 **78.8% 首日浮动 ≈ 0**（开户当天无持仓，符合直觉）。但反过来 **21.2% 不是 0**：

| | 数量 | 占比 |
|---|---|---|
| `\|first_float\| ≥ $100` | 1,728 | 6.9% |
| `\|first_float\| ≥ 5% × avg_bal` | 2,508 | **10.0%** |

**不能简化成「假设期初浮动为 0」** —— 一成客户会算错。

> 待决策：**(a)** 取首个活跃日的真实浮动（我实测的写法）；
> **(b)** 定义成「首个活跃日的前一天 = 0」（等价于把首日浮动也算作本期产生的盈亏）。
> 两者对**老账户**（`stats_balances` 覆盖期开始时已有持仓）含义不同。

### 3. low-equity gate 的阈值

我试的一版是 `avg_eq ≥ 20% × avg_bal` 且 `≥30 活跃天` 且 `avg_eq ≥ $1,000`，拦下 46 个。

⚠ **被拦下的不要渲染成空白** —— 那批恰恰是最该看的（125420 那种）。建议显示明确标记
（如「资本已套牢」）而不是 `—`。

> 待决策：三个阈值各定多少；被拦下时显示什么。

### 4. 是否同时加「扛单率」列

`扛单率 = Avg Daily Floating / Avg Daily Balance`。零额外成本（同一次扫描的副产品）。

它是一个**不依赖序列完整性**的独立信号，可作为 [[OPT-0060]] MDD 的 sanity gate
（MDD 的「已归零账户陷阱」「日内盲区」都会**低估** MDD，而 OPT-0060 的用途是「挑 MDD 小的稳健客户」，
低估 = 危险客户被误标成稳健）。

> 待决策：加 / 不加。

### 5. 新指标 vs ROACE：并列还是替换

**建议并列** —— 两列的差值本身就是扛单强度的直接读数（实测 8.6% 的客户差 ≥20pp）。

> 待决策：并列 / 替换 / 替换但保留 ROACE 在列菜单里默认隐藏。

### 6. 全历史 vs 窗口化

v1 建议**全历史**，与现有 ROACE 对齐，零额外成本。窗口化（像 [[OPT-0060]] 的 5 个窗口）是完全不同量级。

> 待决策：确认 v1 只做全历史。

### 7. 排期：单独做 还是 并进 OPT-0060

三件事用**同一条 `(userId, date)` 序列**：

| | 需要的列 |
|---|---|
| [[OPT-0020]] Drop 2（Sharpe / Consistency） | equity + daily PnL |
| [[OPT-0060]]（MDD ×5 窗口） | equity + 净流入 |
| **本 OPT** | equity + **balance + credit** |

本 OPT 只是**多读两列**。但形态不同：本 OPT 是**服务端 GROUP BY 出 25K 行**（58 秒），
那两个 OPT 规划的是**流式拉全序列到 Python 分组**（2,100 万行过网络）。

> 待决策：**(a)** 单独做，走 GROUP BY（1.5-2 天，收益立即，将来合并时那段 SQL 被替换，
> 但列定义 / schema / 前端 / 测试全部留用）；
> **(b)** 等 OPT-0060 的 7 个开放问题收敛后合并，直接上流式骨架（本 OPT 增量降到约半天，但要等）。
>
> 分析时的建议是 (a)，理由是 OPT-0060 有 3 个会改算法的待拍板问题，等它会拖很久。

---

## 验收标准

> ⚠ 以下 AC 假定开放问题按「推荐项」拍板。用户拍板结果不同则需回来改。
