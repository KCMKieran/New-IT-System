...
- Gap Trade (休市开盘缺口监控): independent Tue-Sat HKT 05:20 cron scanning the **previous MT day's** 00:00-02:00 window (timing intentionally 10 min after the login_ip job's HKT 05:10 run so SO+AB IP enrichment finds a non-empty file). Two sub-rules in one tab — rule 71 (SO + AB pair: 强平 L 腿 + 跨客户同 groupsid 反向 C 腿,IP overlap 高亮); rule 81 (per-client window profit, `userid` 聚合,`profit_ratio_min` 或 `min_profit_usd` 任一触发); day-based time filter (今天/昨天/3d/7d/30d/custom, default Yesterday = where the freshly-scanned data lands); no scan-now / refresh buttons (daily cron only); click row → right-side Sheet detail; `rule_id` 71-90 reserved
