...
- Gap Trade (休市开盘缺口监控): independent Tue-Sat HKT 05:20 cron scanning the **previous MT day's** 00:00-02:00 window (timing intentionally 10 min after the login_ip job's HKT 05:10 run so SO+AB IP enrichment finds a non-empty file). Two sub-rules in one tab — **rule 71「爆仓 AB 仓配对」** (爆仓方 L 腿 stop-out leg + 跨客户同 groupsid 反向对手方 C 腿; `min_l_loss_usd` 默认 $100 砍 dust **但 shared-IP 配对无视该阈值始终保留**; UI 分两段:客户对汇总 + 逐笔明细); **rule 81「Gap Trade 超额获利」** (per-client window profit, `userid` 聚合, `profit_ratio_min` 或 `min_profit_usd` 任一触发,「触发条件」列动态显示当前阈值字符串); day-based time filter **filters on `window_date` (trade day), not `scanned_at`** — `query_alert_events(time_field="window_date")`; default Yesterday = where the freshly-scanned data lands; no scan-now / refresh buttons (daily cron only); click row → right-side Sheet detail; `rule_id` 71-90 reserved. UI 用「爆仓方/对手方」业务术语,DB/SQL/schema 保留 W04 兼容的 `L_*/C_*` 字段名(只在前端做翻译)
