...
_CSV_FIELDS = [
    ...
    "capital_locked",
    # OPT-0060: gated windows are None and must land in the CSV as EMPTY cells,
    # never 0 (0% would read as "most stable"). csv.DictWriter writes None as
    # an empty field, which is exactly right.
    "mdd_30d",
    "mdd_90d",
    "mdd_180d",
    "mdd_365d",
...
def _run_export_task(task_id: str) -> None:
    ...
    try:
        ...
        query_result = get_client_return_rate_data(
            ...
            include_mdd=bool(params.get("include_mdd", True)),
