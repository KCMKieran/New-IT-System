/**
 * 开仓时点扫描 — Window Scan (contract: docs/features/window-scan.md).
 *
 * Pick a concrete HK date-time, get every client who opened a position within
 * ±N minutes of it AND made money on the closed leg. Open positions come along
 * for the ride but never decide inclusion (contract §1).
 *
...
import { ClientDetailSheet } from "./window-scan/ClientDetailSheet";
import { MobileClientList } from "./window-scan/MobileClientList";
import { QueryPanel } from "./window-scan/QueryPanel";
import {
  EmptyState,
  ErrorState,
  IdlePlaceholder,
} from "./window-scan/ResultStates";
import { ClientStatusBadge } from "./window-scan/StatusBadge";
import { FLOATING_CAVEAT } from "./window-scan/TradeDetail";
import {
  buildScanQuery,
  fmtHoldSec,
  fmtInt,
  ...
  return (
    <div className="rounded-xl border bg-card px-4 py-3">
      <div className="grid grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <Stat label="窗口内开仓客户" value={fmtInt(stats.clients_scanned)} />
      ...
      <p className="mt-2 border-t border-border pt-2 text-[11.5px] leading-relaxed text-muted-foreground tabular-nums">
        ...
        窗口 ±{stats.window_min} 分钟 · 分桶 {stats.hold_bucket} · 服务器 sid{" "}
        {stats.sids.join(",")} · 品种 {stats.symbol ?? "全部"}
      </p>
    </div>
  );
}
...
export default function WindowScan() {
  ...
  // anchor / symbol are investigation context — never persisted (contract §6).
  ...
  }, [anchor, windowMin, holdBucket, sids, symbol]);
  ...
  const columnDefs = useMemo<ColDef<ClientRow>[]>(
    () => [
      ...
      },
      {
        ...
        type: "rightAligned",
        headerComponent: InfoHeader,
        headerComponentParams: { tooltip: FLOATING_CAVEAT },
        valueFormatter: (p) =>
          fmtSigned(p.value != null ? Math.round(Number(p.value)) : null),
